...
class DiffusionPriorUNet_p(nn.Module):
    ...
    def forward(self, x, t, c=None):
        ...
        for i in range(self.num_layers-1):
            ...
            scale_x, scale_c, shift = self.encode_time_embedding[i](t).chunk(3, dim=1)
            c_emb = self.encode_cond_embedding[i](c) if c is not None else 0
            x = scale_x * x + scale_c * c_emb + shift
            x = self.encode_layers[i](x)
        
        # 3.3 hidden decoder
        for i in range(self.num_layers-1):
            scale_x, scale_c, shift = self.decode_time_embedding[i](t).chunk(3, dim=1)
...
# def update_ema(model, ema_model, beta):
#     with torch.no_grad():
#         param_dict = dict(model.named_parameters())
#         ema_param_dict = dict(ema_model.named_parameters())

#         for name in param_dict.keys():
#             ema_param_dict[name].data.mul_(beta).add_(param_dict[name].data, alpha=1-beta)
